"""参考脚本：Qwen2-VL + LoRA 的 HF Trainer 图像描述微调（与 DSCA 主链路独立）。"""
import torch
...
def process_func(example):
    """将 COCO 式对话样本转为 Qwen2-VL 训练用 input_ids/attention_mask/labels/pixel 字段。"""
    MAX_LENGTH = 8192
...
def predict(messages, model):
    """单条 chat messages 走 processor + generate 并解码文本。"""
    # 准备推理
    text = processor.apply_chat_template(
        messages, tokenize=False, add_generation_prompt=True
    )
